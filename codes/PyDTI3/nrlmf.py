...
class NRLMF:
    ...
    # def __str__(self):
    #     """Returns also the latent matrices U and V after model training."""
    #     return ("Model: NRLMF, c:%s, K1:%s, K2:%s, r:%s, lambda_d:%s, lambda_t:%s, alpha:%s, beta:%s, theta:%s, max_iter:%s\n"
    #             "Latent matrix U: %s\n"
    #             "Latent matrix V: %s"
    #             % (self.cfix, self.K1, self.K2, self.num_factors, self.lambda_d, self.lambda_t, self.alpha, self.beta, self.theta, self.max_iter,
    #                self.U, self.V))